...
void shuffle_img_1B(unsigned char** img_pointer, int nrows, int ncols, int* grid_dims){
    int numprocs = grid_dims[1]*grid_dims[2];
    unsigned char* img = *img_pointer;
    int proc_coord[2];
    unsigned char* shuffled_img = (unsigned char*)malloc(nrows*ncols*1);
    int index = 0;
    for(int pi = 0; pi < grid_dims[0]; pi++){
        for(int pj = 0; pj < grid_dims[1]; pj++){
            int start_point[2], sub_matrix_sizes[2];
            proc_coord[0] = pi;
            proc_coord[1] = pj;
            //printf("%d %d \n", grid_dims[0], grid_dims[1]);
            //printf("%d %d \n", pi, pj);
            sub_matrix_dim(nrows, ncols, grid_dims, proc_coord, sub_matrix_sizes, start_point);
            for(int i = start_point[0]; i < start_point[0] + sub_matrix_sizes[0]; i++ ){
                for(int j = start_point[1]; j < start_point[1] + sub_matrix_sizes[1]; j++ ){
                    shuffled_img[index] = img[i*ncols + j];
                    index +=1;
                }
            }
...
void de_shuffle_img_1B(unsigned char** img_pointer, int nrows, int ncols, int* grid_dims){
    int numprocs = grid_dims[1]*grid_dims[2];
    unsigned char* img = *img_pointer;
    int proc_coord[2];
    unsigned char* shuffled_img = (unsigned char*)malloc(nrows*ncols*1);
    ...
    for(int pi = 0; pi < grid_dims[0]; pi++){
        for(int pj = 0; pj < grid_dims[1]; pj++){
            ...
            for(int i = start_point[0]; i < start_point[0] + sub_matrix_sizes[0]; i++ ){
                for(int j = start_point[1]; j < start_point[1] + sub_matrix_sizes[1]; j++ ){
                    shuffled_img[i*ncols + j]= img[index];
                    index +=1;
                }
            }
...
void shuffle_img_2B(unsigned short** img_pointer, int nrows, int ncols, int* grid_dims){
    int numprocs = grid_dims[1]*grid_dims[2];
    unsigned short* img = *img_pointer;
    int proc_coord[2];
    unsigned short* shuffled_img = (unsigned short*)malloc(nrows*ncols*2);
    int index = 0;
    for(int pi = 0; pi < grid_dims[0]; pi++){
        for(int pj = 0; pj < grid_dims[1]; pj++){
            int start_point[2], sub_matrix_sizes[2];
...
void de_shuffle_img_2B(unsigned short** img_pointer, int nrows, int ncols, int* grid_dims){
    int numprocs = grid_dims[1]*grid_dims[2];
    unsigned short* img = *img_pointer;
    int proc_coord[2];
    unsigned short* shuffled_img = (unsigned short*)malloc(nrows*ncols*2);
...
int main(int argc, char**argv){
    ...
    for(int p = 0; p < numprocs; p++){
                ...
                chk_sizes[p] = sub_mat_sizes[0]*sub_mat_sizes[1];
                
                displs[p] = pp;
                pp += sub_mat_sizes[0]*sub_mat_sizes[1];
    ...
    switch(1+(maxval > 255)){
      case 1:
      {
        ...
        shuffle_img_1B(&bf_to_send, nrows, ncols, grid_dims);
        ...
        MPI_Scatterv(bf_to_send, chk_sizes, displs, MPI_UNSIGNED_CHAR,my_img, len,  MPI_UNSIGNED_CHAR, 0, grid_comm);
        ...
        MPI_Gatherv(my_img, len, MPI_UNSIGNED_CHAR, final_res, chk_sizes, displs, MPI_UNSIGNED_CHAR, 0, grid_comm);
        
        de_shuffle_img_1B(&final_res, nrows,ncols, grid_dims);
       ...
       //     free(QQ[0]); free(QQ[1]); free(QQ[2]); free(QQ[3]);
        ...
        ////printf("%p %p %p %p\n", QQ[0], QQ[1], QQ[2], QQ[3]);
           ...
           // MPI_Send(my_img, len, MPI_UNSIGNED_CHAR, 0, grid_rank, grid_comm);
            ...
            if (grid_rank == 3) write_pgm_image(my_img, maxval, sub_mat_sizes[1] ,sub_mat_sizes[0],  "cc.pgm");
            //free(my_img);
            //free(res);
            //free(HALOS[0]); free(HALOS[1]); free(HALOS[2]); free(HALOS[3]);
            //free(QQ[0]); free(QQ[1]); free(QQ[2]); free(QQ[3]);
      ...
      case 2:
     {
       ...
       shuffle_img_2B(&bf_to_send, nrows,ncols, grid_dims);
        ...
        MPI_Scatterv(bf_to_send, chk_sizes, displs, MPI_UNSIGNED_SHORT,my_img, len,  MPI_UNSIGNED_SHORT, 0, grid_comm);
        ...
        MPI_Gatherv(my_img, len, MPI_UNSIGNED_SHORT, final_res, chk_sizes, displs, MPI_UNSIGNED_SHORT, 0, grid_comm);
        
        de_shuffle_img_2B(&final_res, nrows,ncols, grid_dims);
        ...
        ////printf("%p %p %p %p\n", QQ[0], QQ[1], QQ[2], QQ[3]);
            ...
            //MPI_Recv(my_img, len, MPI_UNSIGNED_SHORT, 0, grid_rank, grid_comm, &status);
